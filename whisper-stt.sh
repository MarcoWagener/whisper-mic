...
if [ -f "$PID_FILE" ]; then
    ...
    # Route all Whisper errors directly into the log file
    ...
    osascript -e 'display notification "🎤 Initializing AirPods..." with title "Whisper Mic"' 2>>"$LOG_FILE"
    
    echo "[$(date)] Launching FFMPEG..." >> "$LOG_FILE"
    # We replaced "-v quiet" with "-loglevel error" so FFmpeg writes real errors to the log
